---
name: write-spec
description: Generate homework-6/specification.md for the transaction processing pipeline by following the shared template at .claude/templates/specification-template.md. Runs with no arguments and produces the same document the specification-agent does — it reads only the template, never explores the repo, and never assumes anything the template doesn't already settle. Optional arguments supply transaction data, a stack, a framework, or constraints; anything still genuinely open is left as a [NEEDS CLARIFICATION: ...] marker rather than a question. Produces only specification.md, scoped exactly as the template's Scope section defines.
argument-hint: "(optional) transaction data, stack/language, HTTP framework, compliance or risk constraints"
---
...
3. **Never assume, never infer from what happens to exist.** Every stack and framework is whatever `$ARGUMENTS` states and nothing else — the presence of a manifest, a virtualenv, or source files from an earlier run says nothing, and none of them may be consulted. Anything left unstated becomes a `[NEEDS CLARIFICATION: ...]` marker per the template's Gap-Handling Protocol, which alone defines which gaps exist and the exact pinned text for each; never ask, never block, and never enumerate those gaps from here.

4. Apply the template's Scope section to anything `$ARGUMENTS` supplies, exactly as that section defines scope — carrying through what it admits, routing each admitted item to the section or appendix the template assigns it, and silently dropping the rest (dropped material is not a gap). Cite no provenance in the finished document. **What is in scope, and where each thing belongs, is the template's to say and this file's only to obey**; never narrow or widen it from here.
